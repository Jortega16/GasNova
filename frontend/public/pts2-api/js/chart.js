...
(function ($, document) {
    ...
    var u = {
        ...
        fDate: function (d) {
            return addZero(d.getDate()) + "." + addZero(d.getMonth() + 1) + "." + (d.getYear() % 100).toString();
        },
        fTime: function (d) {
        ...
        deltaMinutes: function (min, max) {
            var oneMinute = 60 * 1000; // seconds*milliseconds
            return Math.round(Math.abs((min - max) / (oneMinute)));
        },
        deltaSeconds: function (min, max) {
            var oneSecond = 1000; // seconds*milliseconds
            return Math.round(Math.abs((min - max) / (oneSecond)));
        },
        addZero: function (i) {
    ...
    chart.graphers =
        function (elements, opts, cb) {
            ...
            if (opts.labels.x.show) {
                ...
                minutes = u.deltaMinutes(x.min, x.max)
                if (minutes <= e) {
                    skip = 1;
                } else {
                    skip = ((minutes / e)).toFixed();
                }
                var labelWidth = (width - x.marginLeft - x.margin) / minutes;
                range = x.max - x.min;
                add = range / minutes;
                x.labels = []
                ...
                for (var i = 0; i < minutes; i++) {
                    if (i % skip == 0) {
                        ...
                        context.fillText(u.fDate(new Date(v.value)), x.marginLeft + v.x, height - 10);
                        context.fillText(u.fTime(new Date(v.value)), x.marginLeft + v.x, height);
