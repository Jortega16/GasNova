...
- **Linux:** el host no anuncia su propio nombre por mDNS solo por tener Docker corriendo — para eso el `docker-compose.yml` incluye el servicio `gasnova-mdns` (imagen `flungo/avahi`, `network_mode: host`), detrás de un profile (`mdns`) para no correr un contenedor privilegiado/de red host innecesario en otros sistemas. `installer/install.sh` lo detecta y activa solo en Linux (`docker compose --profile mdns up -d`); si haces la instalación manual en Linux, agrega ese flag tú mismo. Para desactivarlo, comentar el servicio en `docker-compose.yml`.
  - **Excepción — Docker Desktop en Windows (WSL2):** aunque `uname -s` reporte "Linux" dentro de WSL2, `network_mode: host` ahí es la red virtual de la VM de WSL2, no la tarjeta de red física — `gasnova-mdns` arranca sin error pero nunca llega a la LAN real (se registra como `docker-desktop.local`, no `gasnova`). `install.sh` detecta este caso (`uname -r` contiene "microsoft") y no activa el profile. En Windows, incluso corriendo Docker vía WSL2, usar el método de arriba: renombrar la PC como `gasnova` desde la Configuración de **Windows** (no desde dentro de WSL).
